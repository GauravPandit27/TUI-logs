...
class HealthScreen(Screen):
    ...
    def action_export_logs(self) -> None:
        ...
        path = os.path.join(os.getcwd(), f"neolog_export_{ts}.json")
        try:
            with open(path, "w", encoding="utf-8") as f:
                json.dump(rows, f, indent=2, default=str)
            self.notify(f"✅ {len(rows)} logs → neolog_export_{ts}.json", timeout=5)
        except OSError as e:
...
class LogViewerApp(App):
    """NeoLog TUI — real-time structured log viewer."""
